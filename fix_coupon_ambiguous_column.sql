-- ================================================
-- FIX FOR AMBIGUOUS COUPON_ID COLUMN REFERENCE
-- ================================================
-- This fix resolves the PostgreSQL error: 
-- "column reference 'coupon_id' is ambiguous"
-- caused by ambiguous column references in the redeem_coupon function

-- 1. Create a corrected version of the redeem_coupon function
...
-- 3. Test the fix with a simple query to verify it works
-- This should work without the ambiguous column error
-- SELECT * FROM public.redeem_coupon('TEST-DISCOUNT', 'test@example.com', 100000, '[]'::jsonb);

-- 4. Create a simple test function for verification
CREATE OR REPLACE FUNCTION public.test_coupon_fix()
RETURNS TABLE (
  test_name text,
  success boolean,
  message text,
...
AS $$
BEGIN
  -- Test 1: Simple coupon redemption
  RETURN QUERY
  SELECT 
    'Simple coupon test'::text,
...
GRANT EXECUTE ON FUNCTION public.test_coupon_fix() TO anon, authenticated, service_role;