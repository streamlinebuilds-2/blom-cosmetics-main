-- ================================================
-- FRONTEND-BACKEND SYNC FIX - FORCE BACKEND ENFORCEMENT v2
-- ================================================
-- This creates a stronger backend function that the frontend MUST call
-- Fixed ambiguous column reference issues
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT false::boolean, 0::integer, 'none'::text, 0::integer, null::uuid, 'Coupon not found.'::text;
    RETURN;
  END IF;

  -- Check if coupon is active
  IF COALESCE(v_coupon.status, 'inactive') <> 'active' THEN
    RETURN QUERY SELECT false::boolean, 0::integer, 'none'::text, 0::integer, null::uuid, 'Coupon is not active.'::text;
    RETURN;
  END IF;

  -- Check expiry
  IF v_coupon.valid_until IS NOT NULL AND now() > v_coupon.valid_until THEN
    RETURN QUERY SELECT false::boolean, 0::integer, 'none'::text, 0::integer, null::uuid, 'Coupon has expired.'::text;
    RETURN;
  END IF;

  -- Check if used up
  IF COALESCE(v_coupon.used_count, 0) >= COALESCE(v_coupon.max_uses, 1) THEN
    RETURN QUERY SELECT false::boolean, 0::integer, 'none'::text, 0::integer, null::uuid, 'Coupon already used.'::text;
    RETURN;
  END IF;

  -- Check email lock
  IF v_coupon.locked_email IS NOT NULL AND lower(v_coupon.locked_email) <> lower(p_email) THEN
    RETURN QUERY SELECT false::boolean, 0::integer, 'none'::text, 0::integer, null::uuid, 'Coupon locked to another email.'::text;
    RETURN;
  END IF;
  ...
  IF v_eligible_total_cents < v_min_order THEN
    RETURN QUERY SELECT false::boolean, 0::integer, 'none'::text, 0::integer, null::uuid, 
  ...
  ELSE
    RETURN QUERY SELECT false::boolean, 0::integer, 'none'::text, 0::integer, null::uuid, 'Invalid discount type.'::text;
    RETURN;
  END IF;
...
-- 2. Simplified redeem_coupon function (for initial application)
...
BEGIN
  ...
  -- Get min order from coupon (stored in procedure, use default)
  v_min_order := 50000; -- R500 default
...
-- 3. Enhanced recalculate function (for cart changes) - FIXED VERSION
...
SELECT '✅ FRONTEND-BACKEND SYNC FIX v2: All ambiguous column references resolved!' as result;