-- CORRECTED FIX: Stock Deduction When Orders Are Paid
-- This migration uses the ACTUAL database schema with correct column names
...
RETURNS TRIGGER AS $$
BEGIN
  -- Update product inventory_quantity (stock_on_hand)
...
BEGIN
  ...
  IF NEW.product_id IS NOT NULL THEN
    ...
    -- Only validate stock for tracked products that are not courses
    IF (product_record.stock_type = 'tracked' OR product_record.stock_type IS NULL) 
       AND product_record.product_type != 'course' THEN
      -- For products without specific variants
      IF NEW.variant_title IS NULL AND (product_record.stock_on_hand - COALESCE(product_record.stock_reserved, 0)) < NEW.quantity THEN
        RAISE EXCEPTION 'Insufficient stock for product %: available %, requested %', 
          product_record.name, (product_record.stock_on_hand - COALESCE(product_record.stock_reserved, 0)), NEW.quantity;
      END IF;
    END IF;
  END IF;
...
-- Function to get stock movement summary for analytics (FIXED to use correct column names)
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(p.stock_on_hand, 0) as current_stock  -- FIXED: Use stock_on_hand instead of inventory_quantity
  ...
  GROUP BY p.id, p.name, p.stock_on_hand  -- FIXED: Use stock_on_hand instead of inventory_quantity
  ORDER BY p.name;
END;
$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant execute permissions (will be created automatically by SECURITY DEFINER)

-- Create view for easy stock analytics (FIXED to use correct column names)
CREATE OR REPLACE VIEW public.stock_analytics AS
SELECT 
  p.id,
  p.name,
  p.sku,
  COALESCE(p.stock_on_hand, 0) as current_stock,  -- FIXED: Use stock_on_hand
  COALESCE(p.stock_reserved, 0) as reserved_stock,  -- FIXED: Use stock_reserved
  COALESCE(p.stock_available, 0) as available_stock,  -- FIXED: Use stock_available
  CASE WHEN p.stock_type = 'tracked' OR p.stock_type IS NULL THEN true ELSE false END as track_inventory,  -- FIXED: Derive from stock_type
...
FROM public.products p
LEFT JOIN public.stock_movements sm ON sm.product_id = p.id
GROUP BY p.id, p.name, p.sku, p.stock_on_hand, p.stock_reserved, p.stock_available, p.stock_type  -- FIXED: Use correct columns
ORDER BY p.name;
...
-- Migration completed successfully with correct schema
-- This version uses the actual column names: stock_on_hand, stock_reserved, stock_available, stock_type