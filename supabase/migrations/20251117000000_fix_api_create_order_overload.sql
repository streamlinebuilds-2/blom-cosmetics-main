-- Fix api_create_order function overloading issue
-- Drop both versions and create a single function with optional coupon_code parameter

-- Drop the old version WITHOUT coupon_code (14 params)
DROP FUNCTION IF EXISTS public.api_create_order(text, text, text, text, text, text, jsonb, int, int, int, int, text, jsonb, text);

-- Drop the version WITH coupon_code (15 params) if it exists
DROP FUNCTION IF EXISTS public.api_create_order(text, text, text, text, text, text, jsonb, int, int, int, int, text, jsonb, text, text);

-- Create single function with optional p_coupon_code parameter (DEFAULT NULL)
...
BEGIN
  ...
  -- Note: p_coupon_code is accepted but not stored (coupon tracking handled via PayFast custom fields)
